...
var DoubleShelf = function(container2d, app) 
{
    ...
    function reloadConfigurator(item){
        ...
        item.configuration.forEach((itemMain,it)=>{
            if(itemMain.id>0){
            const itemMirror =  item.configuration[it+1];
            add_Conf_stack("right",itemMain,itemMirror);
            }
        })        
    ...
    function CarrentValue (searchFor, thisvalue){
        return arr_build[arr_build.findIndex(e => e.id == searchFor)][thisvalue];
    }
    ...
    function ExtendedBottom(e){
        const mainid = e.target.name.replace(/[^0-9]/g,'');
        let id= +(document.getElementById("arrow"+mainid).dataset.side + mainid);
    ...
    function UpdateInterface(id,width,gridtype,amount, gridheight,borderamount,mianborder,innerborder, hooksA){
        ...
        const mianborderS = document.getElementById("border"+id);
            mianborderS.checked = mianborder;
        const innerborderS = document.getElementById("inner"+id);
            innerborderS.checked = innerborder;
    }
    ...
    function NextObj(e){
        ...
        const side =document.getElementById("arrow"+id).dataset.side;
        const type = e.target.dataset.type;
        const depth =document.querySelector('input[name="depth"]:checked').value;
        let value, itemIndex = listBorders[type].indexOf(CarrentValue (+(side+id),"ObjType"));
        if(listBorders[type][itemIndex+1])
            value=listBorders[type][itemIndex+1];
        else
            value=listBorders[type][0];
        if (side=="+")
            $("#rightSet"+id).attr("src", list[value].imageName);
        else
            $("#leftSet"+id).attr("src", list[value].imageName);   
    ...
    function add_Conf_stack (side="right",item, itemMIrror, ObjType="Type1", width="665",amount="4",gridtype="plastick", gridheight=30, borderamount=2, IsBorders=false, IsInner=false,hooksA=amount)
    {   
        let ObjType2=ObjType;
        if(item){
            ...
            borderamount=item.borderamount; 
            IsBorders=item.IsBorders; 
            IsInner=item.IsInner;
            hooksA=item.hooksA;
            ObjType2=itemMIrror.ObjType;
        }
        ...
        let added=`
            <div class="img_selector-shelf">
            <img class="img_selector-shelf"  id="Img${StackControl}" src="../sprites/configurator/SHELF/Preview/MIan.png">
            <div> &#8203 <br>&#8203<br>&#8203</div>
            <img class="img_selector-shelf-inner-l"  id="leftSet${StackControl}"  src="${list[ObjType2].imageName}">
            <img class="img_selector-shelf-inner-r"  id="rightSet${StackControl}"  src="${list[ObjType].imageName}">
            <img class="img_selector-shelf-inner"  id="arrow${StackControl}" data-side="+" src="../sprites/configurator/SHELF/Preview/Doubleside-right.png">
            </div>
        ...
        if(itemMIrror){
            ...
            amount=itemMIrror.amount;
            gridtype=itemMIrror.gridtype; 
            gridheight=itemMIrror.gridheight; 
            borderamount=itemMIrror.borderamount; 
            IsBorders=itemMIrror.IsBorders; 
        ...
        $("#arrow"+StackControl).click((e)=>{
            ...
            e.target.src= e.target.src.replace(/left/, 'right');
            e.target.dataset.side="+"
            }
            const sideIndex = +(e.target.dataset.side + e.target.id.replace(/[^0-9]/g,''));
            SetInterface(e.target.id.replace(/[^0-9]/g,''),CarrentValue (sideIndex,"ObjType"));
            UpdateInterface(e.target.id.replace(/[^0-9]/g,''),CarrentValue (sideIndex,"width"),CarrentValue (sideIndex,"gridtype"),CarrentValue (sideIndex,"amount"),CarrentValue (sideIndex,"gridheight"),CarrentValue (sideIndex,"borderamount"),CarrentValue (sideIndex,"IsBorders"),CarrentValue (sideIndex,"IsInner"),CarrentValue (sideIndex,"hooksA"))
        });
    }
...
function frontPanelAndLeg(arr,mainObj){
    ...
    arr.push([shelf["leg"+mainObj.userData.height].art,shelf["leg"+mainObj.userData.height].name,leg/2+1,shelf["leg"+mainObj.userData.height].price,shelf["leg"+mainObj.userData.height].price*leg/2+1]);
    arr.push([shelf["legBottomEnd"].art,shelf["legBottomEnd"].name,leg/2+1,shelf["legBottomEnd"].price,shelf["legBottomEnd"].price*leg/2+1]);
    arr.push([shelf["legTopEnd"].art,shelf["legTopEnd"].name,leg/2+1,shelf["legTopEnd"].price,shelf["legTopEnd"].price*leg+1]);
    arr.push([shelf["legconnector"].art,shelf["legconnector"].name,leg+2,shelf["legconnector"].price,shelf["legconnector"].price*leg+2]);
    if(RLeg){
     ...
     arr.push([shelf[name].art,shelf[name].name,mainObj.configuration.length+2,shelf[name].price,shelf[name].price*mainObj.configuration.length+2]);
...
function backWall(arr,mainObj){
    ...
    for(var key in backWallSet){
        arr.push([shelf[key].art,shelf[key].name,backWallSet[key]/2,shelf[key].price,shelf[key].price*backWallSet[key]/2]);
    }  
}
...
function spriteItem(arr_build, colors, height, depth, extBot, x=0, y=0, rot=0) {
    ...
    renderedsprite.name = "DOUBLESHELF";
    ...
    if(extBot) {
        renderedsprite.children[0].y = -64*(+depth+100)/1000;
        renderedsprite.children[1].y = -64*(+depth+100)/1000;
        renderedsprite.children[2].y = 64*(+depth+100)/1000;
        renderedsprite.children[3].y = 64*(+depth+100)/1000;
    } else {
        renderedsprite.children[0].y = -64*(+depth)/1000;
        renderedsprite.children[1].y = -64*(+depth)/1000;
...
function showContextMenu(x,y)
{
    ...
    $("#depthST").text("Depth: " + selectedItem.userData.depth/1000*2 + "m");
    $("#squareST").html("Square: " + Math.round((selectedItem.breadth*selectedItem.userData.depth/1000*2)*100)/100 + "m&#178");
...
function onlongtouch(event) { 
    ...
    for(var i = 1; i<item.configuration.length; i+=2)
    {
...
_group.children.forEach( item => {item.scale.z=-item.scale.z;});

dist = 0;
for(var i = 0; i<item.configuration.length; i+=2)
{
    var currentStack = item.configuration[i];
    if(currentStack.ObjType=='Type1') {
        for(var sh = 0; sh<currentStack.amount; sh++) {
...
DoubleShelf.prototype = Object.create( EventDispatcher.prototype );
DoubleShelf.prototype.constructor = DoubleShelf;
